...
public:
  ...
  OpenGLError(char const *callerName, char const *fnName, ObjectType objType,
              FunctionType fnType);
  virtual ~OpenGLError();
...
// Inline Functions
inline OpenGLError::OpenGLError(char const *callerName, char const *fnName,
                                ObjectType objType, FunctionType fnType)
    : QEvent(type()), m_functionName(fnName), m_callerName(callerName),
      m_objectType(objType), m_functionType(fnType) {}
...
inline OpenGLError::ObjectType OpenGLError::objectType() const {
  return m_objectType;
}
inline OpenGLError::FunctionType OpenGLError::functionType() const {
